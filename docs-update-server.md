---
title: "Update Server"
url: "https://doc.voxta.ai/docs/update-server/"
scraped_at: "2025-12-19T04:37:56.650Z"
---
...
# Update Server
Voxta is regularly updated with fixes and new features.  
As a result it is good practice to upgrade Voxta.  
Here is an overview of the steps to follow in order to safely transfer Voxta’s settings to a new version.
### Upgrade Voxta Server
Download the latest release from [portal.voxta.ai](https://portal.voxta.ai) in the Downloads section (you need to have an active [Patreon](https://doc.voxta.ai/docs/patreon/) subscription).
  1. Close all running instances of Voxta (Voxta Desktop App and Voxta Server).
  2. Extract the files as you would for a [new installation](https://doc.voxta.ai/docs/install-server/) in a new empty folder. Do not overwrite the existing installation to avoid dll conflicts.
  3. Copy the `Data` folder from the previous version to the new one.
     * The `Data/Voxta.db` file contains all of your settings, characters and chat data.
  4. You can now launch the new version!
  5. Update all services in Voxta under `Manage Services` with the `Run All Installers` button. This ensures Python dependencies are also correctly updated.
  6. If everything functions properly, you can delete the folder containing the older version.


Note: If you are using Voxta with [Virt-a-Mate](https://doc.voxta.ai/vam/what-is-virt-a-mate/), don’t forget to [update the plugin](https://doc.voxta.ai/vam/installing-the-plugin/) to the latest version.
[](https://doc.voxta.ai/docs/install-server/) [Next ](https://doc.voxta.ai/docs/menu/)
  *     * [Upgrade Voxta Server](https://doc.voxta.ai/docs/update-server/#upgrade-voxta-server)
